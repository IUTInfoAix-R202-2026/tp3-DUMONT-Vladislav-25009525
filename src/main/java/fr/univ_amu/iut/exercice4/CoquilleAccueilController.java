 ...
 *
 * <ul>
 * <li>injection de {@link Label} via {@code @FXML} (titre central + statut en
 * bas de fenêtre)
 * <li>plusieurs handlers reliés au FXML par {@code onAction="#..."} sur les
 * items d'un {@code
 *       MenuBar}
 * <li>séparation propre : la <b>structure</b> (BorderPane + MenuBar + center +
 * bottom) est dans
 * le FXML, la <b>logique</b> (changement d'écran) est dans le contrôleur
 * </ul>
 *
 * <p>
 * Aucun écran réel n'est implémenté ici : on se contente d'afficher le nom de
 * la rubrique au
 * centre et de tracer le dernier clic dans la barre de statut. C'est suffisant
 * pour vérifier que la
 * navigation principale fonctionne, sans détourner l'exercice de son objectif
 * (FXML déclaratif).
 */
public class CoquilleAccueilController {
  ...
  /**
   * Action du menu « Mes sites ». Affiche le titre correspondant au centre de la
   * coquille.
   */
  @FXML
  ...
  /**
   * Action du menu « Fichier > Quitter ». Ferme la fenêtre courante via la scène
   * du label.
   */
  @FXML
