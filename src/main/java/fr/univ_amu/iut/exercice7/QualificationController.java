...
public class QualificationController {
  ...
  private void initialize() {
    ...
    tableView.getSelectionModel().selectedItemProperty().addListener((obs, oldValue, newValue) -> {
      if (newValue == null) {
        ...
        labelSelection.setText(
            String.format("Séquence %s - %.1f kHz",
                colHorodatage.getCellData(newValue),
                colFrequence.getCellData(newValue)));
      }
    });
    labelSelection.setText("(sélectionnez une séquence dans le tableau)");
    ...
    boutonEcouter.disableProperty().bind(tableView.getSelectionModel().selectedItemProperty().isNull());
  ...
  private void enregistrerVerdict() {
    ...
    // sélectionné.
    if (!choiceBoxVerdict.getValue().isEmpty()) {
